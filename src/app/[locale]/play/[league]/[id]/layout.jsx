...
export default function PlayLayout({ children }) {
  ...
  const { isLoading: isLoadingPackages } = useSelector(
    (state) => state.package,
  );
  ...
  useEffect(() => {
    if (user?.id && currentTeam?.id) {
      dispatch(
        fetchTeamPackages({ user_id: user.id, team_id: currentTeam.id }),
      );
    }
  ...
  return (
    <main className="text-foreground bg-background relative flex min-h-[85vh] flex-col gap-4 overflow-hidden pt-18 pb-2">
